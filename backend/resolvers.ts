...
const getTaskById = async (id: number, db: ServerlessMysql) => {
    const tasks = await db.query<TaskDbQueryResult>('SELECT id,title,status FROM tasks WHERE id = ?', [id]);
...
export const resolvers: Resolvers<ApolloContext> = {
    ...
    Mutation:{
        ...
        async updateTask(parent, args, context){
            ...
            await context.db.query(`UPDATE tasks SET ${columns.join(',')} WHERE id = ?`, sqlParams)
        ...
        async deleteTask(parent, args, context){
            ...
            if(!task){
                throw new UserInputError("Could not find this task")
            }
